...
class Chromatogram:
    ...
    def crop(
        self, time_window: list[float] | None = None, return_df: bool = False
    ) -> None | DataFrame:
        R"""
...
do this before calling `fit_peaks()` or provide the argument `time_window` to the `fit_peaks()`."""
        ...
        self.df = self.df[
            (self.df[self.time_col] >= time_window[0])
    ...
    # TODO: Ideally, this should be broken into simpler pieces.
    def _assign_windows(
        self,
        known_peaks: list = [],
        tolerance: float = 0.5,
        ...
        with warnings.catch_warnings():
            ...
            for i, inds in enumerate([pos_inds, neg_inds]):
                if i == 0:
                    ...
                    _intensity = -intensity
                if len(inds) > 0:
                    __widths, _, _, _ = scipy.signal.peak_widths(
                        _intensity, self._peak_indices, rel_height=0.5
                    )
                    _widths[inds] = __widths[inds]

                    _, _, __left, __right = scipy.signal.peak_widths(
                        _intensity, self._peak_indices, rel_height=rel_height
                    )
        ...
        if len(known_peaks) > 0:
            ...
            # TODO: See if there is a way to make this more streamlined.
            ...
            for i, loc in enumerate(enforced_location_inds):
                autodetected = np.nonzero(
                    np.abs(self._peak_indices - loc) <= (tolerance / self._dt)
                )[0]
        ...
        if len(bg_windows) > 0:
            ...
            if len(split_inds) == 0:
                window_df.loc[
                    window_df["time_idx"].isin(bg_windows["time_idx"].values),
                    "window_id",
                ] = 1
                window_df.loc[
                    window_df["time_idx"].isin(bg_windows["time_idx"].values),
            ...
            bg_ranges = [
                bg_windows.iloc[np.arange(split_inds[i], split_inds[i + 1], 1)][
                    "time_idx"
                ].values
            ...
            for i, rng in enumerate(bg_ranges):
                if len(rng) >= 10:
                    window_df.loc[
                        window_df["time_idx"].isin(rng), "window_id"
                    ] = win_idx
                    window_df.loc[
                        window_df["time_idx"].isin(rng), "window_type"
                    ] = "interpeak"
                    win_idx += 1
        ...
        for g, d in window_df[window_df["window_type"] == "peak"].groupby("window_id"):
            if g > 0:
                ...
                peak_inds = [
                    x for _p in _peaks for x in np.where(self._peak_indices == _p)[0]
                ]
                _dict = {
                    "time_range": d[self.time_col].values,
                    "signal": d[self.int_col].values,
                    "signal_area": d[self.int_col].values.sum(),
                    "num_peaks": len(_peaks),
                    "amplitude": [
                        d[d["time_idx"] == p][self.int_col].values[0] for p in _peaks
                    ],
                    "location": [
                        np.round(
                            d[d["time_idx"] == p][self.time_col].values[0],
                            decimals=self._time_precision,
                        )
                        for p in _peaks
                    ],
    ...
    def _compute_skewnorm(self, x: float | np.ndarray, *params):
        R"""
    ...
    def _fit_skewnorms(self, x: float | np.ndarray, *params):
        R"""
    ...
    def deconvolve_peaks(
        ...
        --------
        peak_props: `dict`
            A dictionary containing properties of the peak fitting procedure.
...
"""
            ...
            for i in range(v["num_peaks"]):
                p0.append(v["amplitude"][i])  # Amplitude parameter
                p0.append(v["location"][i])  # Location parameter
                ...
                _param_bounds = {
                    "amplitude": np.sort(
                        [0.01 * v["amplitude"][i], 100 * v["amplitude"][i]]
                    ),
                    "location": [v["time_range"].min(), v["time_range"].max()],
                    "scale": [0, (v["time_range"].max() - v["time_range"].min()) / 2],
                    "skew": [-np.inf, np.inf],
                }
                ...
                if len(param_bounds) != 0:
                    for p in parorder:
                        if p in param_bounds.keys():
                            if p == "amplitude":
                                _param_bounds[p] = v["amplitude"][i] * np.sort(
                                    param_bounds[p]
                                )
                            elif p == "location":
                                _param_bounds[p] = [
    ...
    def fit_peaks(
        ...
        approx_peak_width: `float`, optional
            ...
            this has no effect. If less than 10-times the time step, an error
            will be thrown and you will be instructed to either i) increase the
        ...
        if correct_baseline and not self._bg_corrected:
            self.correct_baseline(
                window=approx_peak_width, verbose=verbose, return_df=False
            )
    ...
    def correct_baseline(
        ...
        window : `int`
            ...
            the SNIP algorithm. This must be greater than 10 times the time step,
            or else a `ValueError` will be returned.
        return_df : `bool`
        ...
        # Determine if the provided window is at least 10 times the time step.
        # If not, throw an exception
        if (window / self._dt) < 10:
            raise ValueError(
                f"""
The approximate peak width ({window}) is too small, and is comparable is to the
time sampling interval ({self._dt}). Either expand the approximate peak width
or set `correct_baseline=False` to skip this step.
"""
            )
...
\x1b[0m"""
        ...
        if verbose:
            ...
            iter = tqdm.tqdm(
                range(1, n_iter + 1), desc="Performing baseline correction"
            )
        else:
    ...
    def map_peaks(
        ...
        for k, v in params.items():
            ...
            peak_id = (
                np.abs(peak_df["retention_time"].values -
                       ret_time) < loc_tolerance
            )
            ...
            if g in params.keys():
                if "slope" in params[g].keys():
                    ...
                    if "unit" in params[g].keys():
                        peak_df.loc[peak_df["compound"] == g, "unit"] = params[g][
                            "unit"
                        ]
    ...
    def _score_reconstruction(self) -> DataFrame:
        R"""
        ...
        for g, d in self.window_df[self.window_df["window_type"] == "peak"].groupby(
            "window_id"
        ):
            ...
            window_peak_area = (
                np.array(
                    [np.abs(v["reconstructed_signal"])
                     for v in window_peaks.values()]
                ).sum()
                + 1
            )
        ...
        if len(nonpeak) > 0:
            for g, d in nonpeak.groupby("window_id"):
                ...
                recon_area = (
                    np.sum(np.abs(self.unmixed_chromatograms), axis=1)[
                        d["time_idx"].values
                    ].sum()
                    + 1
                )
    ...
    def assess_fit(
        self, rtol: float = 1e-2, fano_tol: float = 1e-2, verbose: bool = True
    ) -> DataFrame:
        R"""
        ...
        score_df = pd.DataFrame([])
        mean_fano = _score_df[_score_df["window_type"] == "peak"][
            "signal_fano_factor"
        ].mean()
        for g, d in _score_df.groupby(["window_type", "window_id"]):
            ...
            tolpass = (
                np.round(
                    np.abs(d["reconstruction_score"].values[0] - 1),
        ...
        print_colors = {
            "valid": ("A+, Success: ", ("black", "on_green")),
...
tolerance `rtol`."""
            ...
            if verbose:
                termcolor.cprint(
                    f"{print_colors[status][0]} Peak Window {int(g)} (t: {d['time_start'].values[0]:0.3f} - {d['time_end'].values[0]:0.3f}) R-Score = {r_score}",
                    *print_colors[status][1],
                    attrs=["bold"],
                    end="",
                )
                print(warning)
...
==========================================
            ...
            for g, d in score_df[score_df["window_type"] == "interpeak"].groupby(
                "window_id"
            ):
                status = d["status"].values[0]
                if status == "valid":
...
to `fit_peaks()`."""
                ...
                if (d["reconstruction_score"].values[0] >= 10) | (
                    ...
                    if d["reconstruction_score"].values[0] == 0:
                        ...
                        r_score = (
                            f"10^{int(np.log10(d['reconstruction_score'].values[0]))}"
                        )
                else:
                    r_score = f"{d['reconstruction_score'].values[0]:0.4f}"
                ...
                if verbose:
                    termcolor.cprint(
                        f"{print_colors[status][0]} Interpeak Window {int(g)} (t: {d['time_start'].values[0]:0.3f} - {d['time_end'].values[0]:0.3f}) R-Score = {r_score} & Fano Ratio = {fano}",
                        *print_colors[status][1],
                        attrs=["bold"],
                        end="",
                    )
            ...
            for g, d in self.peaks.groupby("peak_id"):
                ...
                if self._mapped_peaks is not None:
                    ...
                    if g in self._mapped_peaks.keys():
                        d = self.quantified_peaks[
                            self.quantified_peaks["compound"] == self._mapped_peaks[g]
                        ]
                ...
                ax.fill_between(
                    time,
                    self.unmixed_chromatograms[:, int(g) - 1],
                    label=label,
                    alpha=0.5,
                )
        else:
